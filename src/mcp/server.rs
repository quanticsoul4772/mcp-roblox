...
use crate::mcp::params::{
    ...
    AiFindRelatedParams,
    AiGetContextParams,
    AiIndexProjectParams,
    AiSearchCodebaseParams,
    // Cloud params
    ...
    > RobloxMcpServer<B, L, F, R, W, M, LN, LA>
{
    ...
    /// Get the knowledge graph, returning an error if not configured
    ///
    /// This centralizes the "knowledge graph required" check that all AI tools need.
    /// Returns a reference to the Arc<dyn KnowledgeGraph> if available.
    ///
    /// # Errors
    ...
    > RobloxMcpServer<B, L, F, R, W, M, LN, LA>
{
    ...
    // === TOOLCHAIN TOOLS (8) ===
    // External toolchain integration for StyLua, Rojo, Wally, Moonwave, Lune and other Luau development tools.
    ...
    #[tool(
        description = "Run a Luau script using Lune runtime. Returns stdout, stderr, and exit code. Requires 'lune' to be installed (rokit add lune-org/lune or cargo install lune)."
    )]
    async fn lune_run(
        &self,
        Parameters(params): Parameters<LuneRunParams>,
    ) -> Result<CallToolResult, ErrorData> {
    ...
    #[tool(
        description = "Evaluate inline Luau code using Lune runtime. Creates a temp file and executes it. Useful for quick tests and REPL-like interactions. Requires 'lune' to be installed."
    )]
    async fn lune_eval(
        &self,
        Parameters(params): Parameters<LuneEvalParams>,
    ) -> Result<CallToolResult, ErrorData> {
    ...
    // === AI TOOLS (4) ===
    // These tools provide AI-powered semantic code search using Voyage AI embeddings
    // and Neo4j graph database. Requires the 'ai' feature flag to be enabled.
    //
    // Note: Tools are always compiled but return errors when 'ai' feature is disabled
    // or when the knowledge graph is not configured.

    #[tool(
        description = "Semantic search for Luau scripts using natural language. Returns scripts matching the query by meaning, not just keywords. Requires 'ai' feature and configured Neo4j + Voyage AI."
    )]
    async fn ai_search_codebase(
        &self,
        #[allow(unused_variables)] Parameters(params): Parameters<AiSearchCodebaseParams>,
    ) -> Result<CallToolResult, ErrorData> {
    ...
    #[tool(
        description = "Find scripts related to a given file through code relationships (requires, remote calls, events). Uses graph traversal to discover dependencies. Requires 'ai' feature."
    )]
    async fn ai_find_related(
        &self,
        #[allow(unused_variables)] Parameters(params): Parameters<AiFindRelatedParams>,
    ) -> Result<CallToolResult, ErrorData> {
    ...
    async fn ai_get_context(
        &self,
        #[allow(unused_variables)] Parameters(params): Parameters<AiGetContextParams>,
    ) -> Result<CallToolResult, ErrorData> {
    ...
    async fn ai_index_project(
        &self,
        #[allow(unused_variables)] Parameters(params): Parameters<AiIndexProjectParams>,
    ) -> Result<CallToolResult, ErrorData> {
    ...
    > ServerHandler for RobloxMcpServer<B, L, F, R, W, M, LN, LA>
{
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            instructions: Some(
                "Roblox Studio MCP Server. Provides 7 filesystem tools for .luau script management (including Selene linting), 9 Studio bridge tools for live Roblox Studio interaction (including paginated DataModel), 3 Open Cloud tools (publish, asset upload, datastore) for CI/CD automation, 1 file watcher tool for change detection, 2 Lune tools for local Luau script execution, and 1 metrics tool for monitoring. Studio tools require the plugin to be connected. Cloud tools require ROBLOX_OPEN_CLOUD_API_KEY."
                    .into(),
            ),
